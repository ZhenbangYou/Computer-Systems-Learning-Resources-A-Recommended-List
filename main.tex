...
\centerline{\large{Last Updated: May 27, 2022}}
...
\textbf{Tips}
\begin{enumerate}
    \item You need not only \emph{compilers}, \emph{libraries} and \emph{IDE}, but also \emph{build tools}, \emph{debuggers}, \emph{test tools}, \emph{linters}, and even \emph{sanitizers}, \emph{profilers}, \emph{formatters}, etc.
    Development toolchains matter.
    \item Frameworks are key components of the ecosystem of a PL.
    Success of some PL highly relies on the its frameworks.
    \item Sometimes searching within a tutorial/documentation returns better result than searching \emph{Google}.
    ...
    Generally speaking, JetBrains and VSCode are always good choices (except that the VSCode support for JVM is awful).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Scala} (\emph{Scala 3} is here!)\\
    Prerequisite languages: Java.\\
    Noteworthy features: FP.\\
    Multi-paradigmatic languages like \emph{Scala} and \emph{OCaml} is great starting points to learn \emph{functional programming}, since your programming style can transition gradually from \emph{imperative programming} to \emph{functional programming}.
    \begin{itemize}
        \item Tutorials
        \begin{enumerate}
            \item Tour of Scala:
        ...
        \item Currently, the ecosystem of \emph{Scala 2} is better than that of \emph{Scala 3}.
        The development \emph{Scala 3} is still at the early stage, but it is also recommended, especially in the long run.
        \end{itemize}
        
    \item \textbf{Kotlin}
    (\emph{Kotlin/JVM} is recommended, with \emph{Kotlin/Native} and \emph{Kotlin/JS} being immature)\\
    ...
    \item \textbf{Go} (\emph{Go 1.18} is here!)\\
    ...
    Do reflect on \emph{C++} when learning Rust.
    \begin{itemize}
        \item Tutorials
        \begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \emph{Type inference} of \emph{OCaml} is even more powerful than that of \emph{Scala}, e.g., in \emph{Scala}, you need to specify the return value of recursive functions.
        \item Arguments are passed by \emph{reference} in \emph{Scala}, and by \emph{value} in \emph{OCaml}.
    \end{enumerate}
    ...
    \emph{Scala} $\rightarrow$ \emph{OCaml} $\rightarrow$ \emph{Haskell}.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}
                ...
                \item The syntax of \emph{MySQL} is wierd sometimes.
            \end{enumerate}
